defmodule Leader do
  ...
  defp pmax(pvals) do
    ...
    slot_to_command =
      List.foldl(Map.to_list(slot_to_ballot_and_command), Map.new(),
        fn ({slot, {ballot, cmd}}, slot_to_cmd) ->
  ...
  defp next(ballot, active, proposals, replicas, acceptors) do
    {active, ballot, proposals} = receive do
      {:propose, slot, cmd} ->
        {active, ballot,
          Map.put_new_lazy(proposals, slot, fn() ->
            if active, do: spawn(Commander, :start,
